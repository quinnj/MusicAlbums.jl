...
function createAlbum(name, artist, year, songs)
    ...
    resp = HTTP.post(string(SERVER[], "/album"), [], JSON3.write(body); cookies=true)
    return JSON3.read(resp.body, Album)
end

function getAlbum(id)
    resp = HTTP.get(string(SERVER[], "/album/$id"); cookies=true)
    return JSON3.read(resp.body, Album)
end

function updateAlbum(album)
    resp = HTTP.put(string(SERVER[], "/album/$(album.id)"), [], JSON3.write(album); cookies=true)
    return JSON3.read(resp.body, Album)
end

function deleteAlbum(id)
    resp = HTTP.delete(string(SERVER[], "/album/$id"); cookies=true)
    return
end

function pickAlbumToListen()
    resp = HTTP.get(string(SERVER[], "/"); cookies=true)
    return JSON3.read(resp.body, Album)
end
